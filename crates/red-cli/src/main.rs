...
USAGE:
    red [--allow-shell] [--walk] <file.red> [args...]   Load and evaluate a Red source file
...
(useful for debugging and parity comparison). Script output (native
`print`/`prin`/`probe`) goes to stdout; the final value is not printed by
the CLI (use `print` in the script). Errors print to stderr as
...
fn main() -> ExitCode {
    ...
    match positional.as_slice() {
        ...
        [path, rest @ ..] => run_file(path, rest, allow_shell, walk),
    }
}

fn run_file(path: &str, args: &[String], allow_shell: bool, walk: bool) -> ExitCode {
